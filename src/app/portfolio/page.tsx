...
export default function Portfolio() {
  return (
    <div className="page-fade-in">
      <div className="w-full md:w-full max-w-6xl lg:max-w-7xl xl:max-w-[1200px] 2xl:max-w-[1560px] mx-auto flex flex-col gap-4 sm:gap-6 md:gap-8 lg:gap-10 p-2 sm:px-4 md:px-6 lg:px-8 xl:px-12 py-1 pb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 md:gap-8 mt-8 sm:mt-12">
          {expertiseData.map((item, index) => (
            <div
              key={index}
              className="bg-card border border-card-border p-4 sm:p-6 md:p-8 lg:p-10 rounded-xl sm:rounded-2xl lg:rounded-3xl shadow-sm hover:shadow-xl transition-shadow duration-300 cursor-pointer"
            >
              <h3 className="text-lg sm:text-xl md:text-2xl font-bold mb-3 text-primary">
                {item.title}
              </h3>
