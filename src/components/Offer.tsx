...
const Offer = () => {
  ...
  return (
    <section
      id="services"
      className="w-full max-w-6xl lg:max-w-7xl xl:max-w-6xl 2xl:max-w-7xl mx-auto bg-card border border-card-border p-2 sm:p-4 md:p-6 lg:p-8 xl:p-10 2xl:p-8 py-8 sm:py-12 md:py-16 2xl:py-12 rounded-xl sm:rounded-2xl lg:rounded-3xl 2xl:rounded-3xl relative overflow-hidden shadow-sm"
    >
      ...
      <div className="relative flex flex-col items-center justify-center gap-3 text-center">
        <p className="text-primary flex gap-0.5 justify-center items-center text-xs sm:text-sm md:text-base 2xl:text-base font-semibold">
          <GoDotFill className="animate-pulse" /> {c.tagline}
        </p>
        <h2 className="text-2xl sm:text-4xl md:text-5xl lg:text-3xl xl:text-4xl 2xl:text-3xl font-bold leading-tight 2xl:leading-tight">
          {c.heading}{" "}
          <span className="text-slate-500">{c.highlight}</span>
        </h2>
        <p className="text-sm sm:text-base md:text-lg 2xl:text-base text-slate-400 dark:text-slate-500 max-w-xl 2xl:max-w-5xl mx-auto 2xl:leading-loose 2xl:mt-4">
          {c.sub}
        </p>
      </div>

      {/* Tab Controls: Segmented navigation for category selection */}
      <div className="relative flex justify-center mt-10 2xl:mt-12">
        <div className="inline-flex gap-1 p-1 2xl:p-1.5 rounded-2xl 2xl:rounded-2xl bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-inner">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActive(tab.id)}
              className={`flex items-center gap-2 2xl:gap-3 px-4 sm:px-6 2xl:px-8 py-2.5 2xl:py-3 rounded-xl 2xl:rounded-xl text-sm 2xl:text-base font-bold transition-all duration-300 cursor-pointer
                ${active === tab.id
      ...
      <div className="relative flex flex-wrap justify-center gap-3 sm:gap-4 2xl:gap-4 mt-10 2xl:mt-12">
        {c.platforms.map((p) => (
          <div
            key={p.label}
            className={`flex items-center gap-2 2xl:gap-3 px-4 2xl:px-6 py-2 2xl:py-3 rounded-2xl 2xl:rounded-[2rem] border ${p.bg} ${p.color} hover:scale-105 transition-transform duration-200`}
          >
            <span className="2xl:scale-110 transform transition-transform origin-center">{p.icon}</span>
            <span className="font-bold text-sm sm:text-base 2xl:text-base">{p.label}</span>
          </div>
        ))}
      </div>

      {/* Feature Grid: Detailed value propositions for the active category */}
      <div className="relative grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 2xl:gap-6 mt-10 2xl:mt-12">
        {c.features.map((feat, i) => (
          <div
            key={i}
            className="flex flex-col gap-3 p-4 sm:p-6 2xl:p-8 rounded-2xl 2xl:rounded-3xl border border-card-border bg-card/60 hover:border-primary/40 hover:bg-primary/5 transition-all duration-300 group shadow-sm hover:shadow-md cursor-pointer"
          >
            <div className="w-12 h-12 2xl:w-14 2xl:h-14 flex items-center justify-center rounded-xl 2xl:rounded-2xl bg-card border border-card-border group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 shadow-inner">
              <span className="2xl:scale-110 origin-center">{feat.icon}</span>
            </div>
            <h4 className="font-bold text-sm sm:text-base 2xl:text-lg text-zinc-800 dark:text-zinc-100">{feat.title}</h4>
            <p className="text-xs sm:text-sm 2xl:text-sm text-slate-500 dark:text-slate-400 leading-relaxed 2xl:leading-loose">
              {feat.description}
            </p>
          </div>
        ))}
      ...
      <div className="relative mt-10 2xl:mt-12 flex flex-col sm:flex-row items-center justify-between gap-4 p-6 2xl:p-8 rounded-2xl 2xl:rounded-3xl border border-primary/20 bg-primary/5 shadow-inner">
        <div className="flex items-center gap-4 2xl:gap-6">
          <div className="hidden sm:block p-3 2xl:p-4 rounded-full bg-white dark:bg-zinc-800 shadow-sm">
            <span className="2xl:scale-125 inline-block origin-center">{c.cta.icon}</span>
          </div>
          <div>
            <p className="font-bold text-sm sm:text-base 2xl:text-xl text-zinc-800 dark:text-zinc-100">
              {c.cta.title}
            </p>
            <p className="text-xs sm:text-sm 2xl:text-base text-slate-500 dark:text-slate-400">
              {c.cta.body}
            </p>
          </div>
        </div>
        <a
          ...
          className="flex-shrink-0 px-8 2xl:px-10 py-3 2xl:py-4 rounded-full bg-primary text-white font-black text-sm 2xl:text-lg hover:bg-primary/90 transition-all duration-200 shadow-lg shadow-primary/20 hover:-translate-y-0.5 active:translate-y-0"
        >
          {c.cta.btn}
        </a>
      </div>
