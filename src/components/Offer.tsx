...
const Offer = () => {
  ...
  return (
    <section
      id="services"
      className="w-full md:w-full max-w-6xl lg:max-w-7xl xl:max-w-[1200px] 2xl:max-w-[1560px] mx-auto bg-card border border-card-border p-2 sm:p-4 md:p-6 lg:p-8 xl:p-10 py-8 sm:py-12 md:py-16 rounded-xl sm:rounded-2xl lg:rounded-3xl relative overflow-hidden shadow-sm"
    >
      ...
      <div className="relative flex flex-col items-center justify-center gap-3 text-center">
        ...
        <h2 className="text-2xl sm:text-4xl md:text-5xl lg:text-3xl xl:text-4xl font-bold leading-tight">
          {c.heading}{" "}
          <span className="text-slate-500">{c.highlight}</span>
        </h2>
        <p className="text-sm sm:text-base md:text-lg text-slate-400 dark:text-slate-500 max-w-xl mx-auto">
          {c.sub}
        </p>
      </div>
        ...
        <div className="inline-flex gap-1 p-1 rounded-2xl bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-inner">
          {tabs.map((tab) => (
            <button
              ...
              className={`flex items-center gap-2 px-4 sm:px-6 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 cursor-pointer
                ${active === tab.id
      ...
      <div className="relative flex flex-wrap justify-center gap-3 sm:gap-4 mt-10">
        {c.platforms.map((p) => (
          ...
          >
            <span className="transform transition-transform origin-center">{p.icon}</span>
      ...
      <div className="relative grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-10">
        {c.features.map((feat, i) => (
          <div
            key={i}
            className="flex flex-col gap-3 p-4 sm:p-6 rounded-2xl border border-card-border bg-card/60 hover:border-primary/40 hover:bg-primary/5 transition-all duration-300 group shadow-sm hover:shadow-md cursor-pointer"
          >
            <div className="w-12 h-12 flex items-center justify-center rounded-xl bg-card border border-card-border group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 shadow-inner">
              <span className="origin-center">{feat.icon}</span>
            </div>
      ...
      <div className="relative mt-10 flex flex-col sm:flex-row items-center justify-between gap-4 p-6 rounded-2xl border border-primary/20 bg-primary/5 shadow-inner">
        <div className="flex items-center gap-4">
          <div className="hidden sm:block p-3 rounded-full bg-white dark:bg-zinc-800 shadow-sm">
            <span className="inline-block origin-center">{c.cta.icon}</span>
          </div>
          <div>
