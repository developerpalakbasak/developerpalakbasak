...
const UnifiedSkills = () => {
  ...
  return (
    <section className="w-full max-w-6xl lg:max-w-7xl xl:max-w-6xl 2xl:max-w-7xl mx-auto p-2 sm:p-4 md:p-6 lg:p-8 xl:p-10 2xl:p-8 py-8 sm:py-12 md:py-16 2xl:py-12 bg-card border border-card-border rounded-xl sm:rounded-2xl lg:rounded-3xl 2xl:rounded-3xl relative overflow-hidden shadow-sm">
      ...
      <div className="flex flex-row w-[95%] sm:w-fit p-1.5 bg-card-secondary/50 backdrop-blur-md border border-card-border rounded-full mx-auto mb-8 sm:mb-12 shadow-inner overflow-hidden">
        {skillCategories.map((cat) => (
          <button
            key={cat.id}
            onClick={() => setActiveTab(cat.id)}
            className={`flex-1 sm:flex-initial flex items-center justify-center gap-2 md:gap-3 2xl:gap-4 px-4 md:px-10 2xl:px-12 py-2.5 md:py-4 2xl:py-3 rounded-full font-extrabold text-xs md:text-base 2xl:text-lg transition-all duration-500 cursor-pointer ${activeTab === cat.id
          ...
          >
            <span className="text-xl sm:text-2xl 2xl:text-2xl">{cat.icon}</span>
      ...
      <div className="p-0 sm:p-2 md:p-4 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          {skillCategories.filter(c => c.id === activeTab).map(cat => (
            <div key={cat.id} className="space-y-8">
              <div className="space-y-4">
                ...
                <h3 className="text-xl sm:text-3xl lg:text-4xl xl:text-4xl 2xl:text-3xl font-black tracking-tight text-slate-900 dark:text-white leading-tight">{cat.subtitle}</h3>
                <p className="text-xs sm:text-base lg:text-lg xl:text-lg 2xl:text-lg 2xl:leading-loose text-slate-600 dark:text-slate-400 leading-relaxed font-medium">
                  {cat.description}
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {cat.skills.map((skill, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-4 2xl:gap-6 p-2 sm:p-5 2xl:p-4 rounded-xl sm:rounded-2xl lg:rounded-3xl 2xl:rounded-2xl bg-card border border-card-border hover:border-primary/50 hover:bg-primary/5 transition-all duration-300 group shadow-sm hover:shadow-primary/5"
                  >
                    <div className="w-12 h-12 2xl:w-12 2xl:h-12 flex items-center justify-center rounded-xl sm:rounded-2xl lg:rounded-3xl 2xl:rounded-3xl bg-card-secondary border border-card-border text-2xl 2xl:text-2xl text-primary group-hover:scale-110 group-hover:rotate-6 transition-all duration-300 shadow-inner">
                      {skill.icon}
                    </div>
                    <span className="font-bold 2xl:text-lg text-slate-800 dark:text-slate-200">{skill.name}</span>
                  </div>
                ))}
              </div>

              {/* Capabilities Checklist: Key value adds for the category */}
              <div className="p-6 2xl:p-6 rounded-xl sm:rounded-2xl lg:rounded-3xl 2xl:rounded-2xl bg-primary/5 border border-primary/10 space-y-4 2xl:space-y-4">
                <h4 className="font-bold text-primary flex items-center gap-2 uppercase tracking-widest text-xs 2xl:text-base">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 gap-x-6">
                  {cat.features.map((f, i) => (
                    <div key={i} className="flex items-center gap-3 text-slate-700 dark:text-slate-300 group cursor-default">
                      ...
                      <span className="text-sm md:text-base 2xl:text-base font-medium">{f}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="lg:col-span-6 flex justify-center order-1 lg:order-2">
          <div className="relative w-full max-w-[280px] sm:max-w-[400px] aspect-square flex items-center justify-center">
            ...
            <div className="relative z-10 w-44 h-44 sm:w-60 sm:h-60 2xl:w-48 2xl:h-48 rounded-full bg-card border-2 border-card-border flex flex-col items-center justify-center group hover:border-primary/40 transition-all duration-700 shadow-2xl overflow-hidden">
              {/* Inner glow pulse */}
              <div className="absolute inset-0 bg-gradient-to-br from-primary/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700" />

              <div className="text-6xl sm:text-8xl 2xl:text-[6rem] text-primary mb-3 2xl:mb-4 transition-all duration-700 group-hover:scale-110 group-hover:rotate-3">
                {skillCategories.find(c => c.id === activeTab)?.icon}
              </div>

              <div className="text-center relative z-10">
                <p className="font-black text-xl sm:text-2xl 2xl:text-2xl uppercase tracking-[0.3em] text-slate-900 dark:text-white group-hover:tracking-[0.4em] transition-all duration-700">
                  {activeTab}
                </p>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
          <div className="flex items-center gap-3 sm:gap-4">
            {[FaGlobe, FaMobileAlt, FaDesktop].map((Icon, i) => (
              <div key={i} className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 2xl:w-12 2xl:h-12 rounded-xl sm:rounded-2xl lg:rounded-3xl 2xl:rounded-3xl bg-card-secondary border border-card-border flex items-center justify-center text-xl sm:text-2xl 2xl:text-2xl text-primary shadow-lg hover:-translate-y-1 transition-transform duration-300">
                <Icon />
              </div>
            ))}
          </div>
          <div className="text-center sm:text-left">
            <p className="text-lg sm:text-xl 2xl:text-xl font-bold text-slate-900 dark:text-white leading-tight">Universal Architecture</p>
            <p className="text-xs sm:text-sm md:text-base 2xl:text-base text-slate-600 dark:text-slate-400">One solution for every screen, built to scale.</p>
          </div>
        </div>

        <a
          href="#contact"
          className="w-full sm:w-auto group relative px-8 sm:px-10 2xl:px-10 py-3 sm:py-4 2xl:py-4 bg-primary text-white font-black rounded-xl sm:rounded-2xl 2xl:rounded-xl shadow-[0_10px_30px_rgba(var(--primary-rgb),0.3)] hover:shadow-[0_15px_40px_rgba(var(--primary-rgb),0.4)] transition-all duration-300 hover:-translate-y-1 overflow-hidden flex items-center justify-center text-sm sm:text-base 2xl:text-lg"
        >
