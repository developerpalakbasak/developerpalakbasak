...
const UnifiedSkills = () => {
  ...
  return (
    <section className="w-full md:w-full max-w-6xl lg:max-w-7xl xl:max-w-[1200px] 2xl:max-w-[1560px] mx-auto p-2 sm:p-4 md:p-6 lg:p-8 xl:p-10 py-8 sm:py-12 md:py-16 bg-card border border-card-border rounded-xl sm:rounded-2xl lg:rounded-3xl relative overflow-hidden shadow-sm">
      ...
      <div className="relative z-10 flex flex-col items-center mb-16">
        ...
        <h2 className="text-2xl sm:text-4xl md:text-4xl font-black text-center mb-4 sm:mb-6 leading-tight">
          Mastering the <span className="text-gradient">Digital Realm</span>
        </h2>
        <p className="text-slate-600 dark:text-slate-400 text-center max-w-3xl text-sm sm:text-lg md:text-xl leading-relaxed">
      ...
      <div className="flex flex-row w-[95%] sm:w-fit p-1.5 bg-card-secondary/50 backdrop-blur-md border border-card-border rounded-full mx-auto mb-8 sm:mb-12 shadow-inner overflow-hidden">
        {skillCategories.map((cat) => (
          <button
            ...
            className={`flex-1 sm:flex-initial flex items-center justify-center gap-2 md:gap-3 px-4 md:px-10 py-2.5 md:py-4 rounded-full font-extrabold text-xs md:text-base transition-all duration-500 cursor-pointer ${activeTab === cat.id
          ...
          {skillCategories.filter(c => c.id === activeTab).map(cat => (
            <div key={cat.id} className="space-y-8">
              <div className="space-y-4">
                ...
                <h3 className="text-xl sm:text-3xl lg:text-4xl xl:text-4xl font-black tracking-tight text-slate-900 dark:text-white leading-tight">{cat.subtitle}</h3>
                <p className="text-xs sm:text-base lg:text-lg xl:text-lg text-slate-600 dark:text-slate-400 leading-relaxed font-medium">
                  {cat.description}
                </p>
              </div>
