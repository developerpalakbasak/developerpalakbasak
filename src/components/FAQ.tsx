  ...
  return (
    <section className="w-full max-w-6xl lg:max-w-7xl xl:max-w-6xl mx-auto p-2 sm:p-4 md:p-6 lg:p-8 xl:p-10 py-8 sm:py-12 md:py-16 bg-card/80 backdrop-blur-sm border border-card-border rounded-xl sm:rounded-2xl lg:rounded-3xl relative overflow-hidden shadow-sm">
      ...
      <div className="relative z-10 flex flex-col items-center mb-6 sm:mb-12">
        ...
        <h2 className="text-2xl sm:text-4xl md:text-4xl font-black text-center mb-4">
        ...
        <p className="text-slate-500 dark:text-slate-400 text-center max-w-2xl text-sm sm:text-lg md:text-xl">
        ...
        {faqData.map((faq, index) => (
          ...
          >
            ...
            <button
              ...
              className="w-full flex items-center justify-between p-3 sm:p-4 md:p-6 lg:p-8 text-left transition-colors cursor-pointer"
            >
              <div className="flex items-start gap-3 sm:gap-4">
                <div className={`w-8 h-8 sm:w-10 sm:h-10 shrink-0 flex items-center justify-center rounded-xl sm:rounded-2xl lg:rounded-3xl border border-card-border mt-0.5 transition-all duration-300 ${openIndex === index ? "bg-primary text-white scale-110 shadow-lg shadow-primary/20" : "bg-card-secondary text-primary"
                  }`}>
                  <FaQuestionCircle className="text-base sm:text-xl" />
                </div>
                <span className={`text-base md:text-xl font-bold transition-colors ${openIndex === index ? "text-slate-900 dark:text-white" : "text-slate-600 dark:text-slate-300 group-hover:text-slate-900 dark:group-hover:text-white"
                  }`}>
                  {faq.question}
                </span>
              </div>
