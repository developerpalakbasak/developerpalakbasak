...
const FAQ = () => {
  ...
  return (
    <section className="w-full max-w-6xl lg:max-w-7xl xl:max-w-6xl 2xl:max-w-7xl mx-auto p-2 sm:p-4 md:p-6 lg:p-8 xl:p-10 2xl:p-8 py-8 sm:py-12 md:py-16 2xl:py-12 bg-card/80 backdrop-blur-sm border border-card-border rounded-xl sm:rounded-2xl lg:rounded-3xl 2xl:rounded-3xl relative overflow-hidden shadow-sm">
        ...
        {faqData.map((faq, index) => (
          ...
          >
            ...
            <button
              ...
              className="w-full flex items-center justify-between p-3 sm:p-4 md:p-6 lg:p-8 2xl:p-6 text-left transition-colors cursor-pointer"
            >
              <div className="flex items-start gap-3 sm:gap-4">
                <div className={`w-8 h-8 sm:w-10 sm:h-10 2xl:w-10 2xl:h-10 shrink-0 flex items-center justify-center rounded-xl sm:rounded-2xl lg:rounded-3xl 2xl:rounded-2xl border border-card-border mt-0.5 transition-all duration-300 ${openIndex === index ? "bg-primary text-white scale-110 shadow-lg shadow-primary/20" : "bg-card-secondary text-primary"
                  }`}>
                  <FaQuestionCircle className="text-base sm:text-xl 2xl:text-lg" />
                </div>
                <span className={`text-base md:text-xl 2xl:text-lg font-bold transition-colors ${openIndex === index ? "text-slate-900 dark:text-white" : "text-slate-600 dark:text-slate-300 group-hover:text-slate-900 dark:group-hover:text-white"
                  }`}>
                  {faq.question}
                </span>
              </div>
            ...
            >
              <div className="px-5 sm:px-8 2xl:px-8 pb-6 sm:pb-8 2xl:pb-8 pt-2">
                <div className="h-[1px] w-full bg-card-border mb-4 sm:mb-6" />
                <p className="text-slate-600 dark:text-slate-400 text-sm sm:text-lg 2xl:text-base leading-relaxed 2xl:leading-loose md:pl-14 2xl:pl-12">
                  {faq.answer}
                </p>
              </div>
            </div>
