...
const SingleProject = ({ project, index, islast }) => {
  return (
    <section className={`border-b border-card-border last:border-b-0`}>
      ...
      <div className={`w-full md:w-full max-w-6xl lg:max-w-7xl xl:max-w-[1200px] 2xl:max-w-[1560px] mx-auto mt-5 p-2 sm:p-4 md:p-6 lg:p-8 xl:p-10 py-8 sm:py-12 md:py-16 bg-card/30 rounded-xl sm:rounded-2xl lg:rounded-3xl flex gap-4 sm:gap-8 flex-col ${index % 2 === 0 ? "md:flex-row-reverse" : "md:flex-row"}`}>
        ...
        <div className="w-full md:w-[60%] flex flex-col justify-center">
          <h2 className="text-lg sm:text-2xl md:text-2xl lg:text-3xl xl:text-3xl font-bold leading-tight">
            {project.title}{" - "} <span className="text-primary">{project.titleQute}</span>
          </h2>
          <p className="my-4 text-xs sm:text-base lg:text-base xl:text-base text-slate-500 dark:text-slate-400 leading-relaxed font-medium">
            {project.description}
          </p>
