...
import { FaReact, FaNodeJs, FaCss3Alt, FaHtml5, FaLinkedinIn } from "react-icons/fa";
import { SiExpress, SiMongodb, SiTailwindcss, SiRefinedgithub, SiUpwork } from "react-icons/si";
import { TbBrandNextjs } from "react-icons/tb";
import { motion } from "framer-motion";

const Hero = () => {
  const icons = [
    { icon: <FaLinkedinIn />, to: "https://www.linkedin.com/in/palak-basak-a05510208/" },
    { icon: <SiRefinedgithub />, to: "https://github.com/developerpalakbasak" },
    { icon: <SiUpwork />, to: "https://www.upwork.com/freelancers/~01497f6abe72a2a2ac" },
  ];
      ...
      >
        ...
        >
          ...
          <h1 className="text-3xl">
            Senior<span className="text-primary">{`${" "}{ Full-Stack } ${" "}`}</span>
            <span>Developer</span>
          </h1>
        </motion.div>
        ...
        <motion.p
          ...
          <span className="text-red-700">MongoDB, Express, React, NodeJS,</span> and{" "}
          <span className="text-red-700">NEXTJS</span> ... I deliver web solutions that are both innovative and robust.
        </motion.p>
          ...
          <div className="flex gap-4 flex-wrap">
            {primaryIcons.map((item, index) => (
              <motion.span
                initial={{ y: 100, opacity: 0 }}
                whileInView={{ y: 0, opacity: 1 }}
                transition={{ duration: 0.40 + index * 0.05 }}
                viewport={{ once: true, amount: 0 }}
                key={index}
